      ...
      <div class="tools" @mousedown.stop>
        <div class="tool-content">
          <Popover placement="top" trigger="manual" :value="sizePopoverType === 'pen'" @hide="sizePopoverType = ''">
            <template #content>
              <div class="setting">
                <div class="label">{{ $t('writingBoard.thickness') }}</div>
                <Slider class="size-slider" :min="4" :max="10" :step="2" v-model:value="penSize" />
              </div>
            </template>
            <div class="btn" :class="{ 'active': writingBoardModel === 'pen' }" v-tooltip="$t('writingBoard.pen')" @click="changeModel('pen')">
          ...
          <Popover placement="top" trigger="manual" :value="sizePopoverType === 'shape'" @hide="sizePopoverType = ''">
            ...
            <div class="btn" :class="{ 'active': writingBoardModel === 'shape' }" v-tooltip="$t('writingBoard.shape')" @click="changeModel('shape')">
          ...
          <Popover placement="top" trigger="manual" :value="sizePopoverType === 'mark'" @hide="sizePopoverType = ''">
            ...
            <div class="btn" :class="{ 'active': writingBoardModel === 'mark' }" v-tooltip="$t('writingBoard.highlight')" @click="changeModel('mark')">
          ...
          <Popover placement="top" trigger="manual" :value="sizePopoverType === 'eraser'" @hide="sizePopoverType = ''">
            <template #content>
              <div class="setting">
                <div class="label">{{ $t('writingBoard.eraserSize') }}</div>
                <Slider class="size-slider" :min="20" :max="200" :step="20" v-model:value="rubberSize" />
              </div>
            </template>
            <div class="btn" :class="{ 'active': writingBoardModel === 'eraser' }" v-tooltip="$t('writingBoard.eraser')" @click="changeModel('eraser')">
              <IconErase class="icon" />
            </div>
          </Popover>
          <div class="btn" v-tooltip="$t('writingBoard.clear')" @click="clearCanvas()">
            <IconClear class="icon" />
          </div>
          <div class="btn" :class="{ 'active': blackboard }" v-tooltip="$t('writingBoard.blackboard')" @click="blackboard = !blackboard">
            <IconFill class="icon" />
          </div>
          <div class="colors">
            <div 
        ...
        <div class="btn close" v-tooltip="$t('writingBoard.close')" @click="closeWritingBoard()">
