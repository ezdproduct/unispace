<template>
  <div class="slide-design-panel">
    <div class="title">{{ $t('toolbar.backgroundFill') }}</div>
    <div class="row">
      <Select 
        style="flex: 1;" 
      ...
      <Select 
        ...
        :options="[
          { label: $t('toolbar.contain'), value: 'contain' },
          { label: $t('toolbar.repeat'), value: 'repeat' },
          { label: $t('toolbar.cover'), value: 'cover' },
        ]"
      />
    ...
    <div class="background-gradient-wrapper" v-if="background.type === 'gradient'">
      ...
      <div class="row">
        <div style="width: 40%;">{{ $t('toolbar.currentGradient') }}：</div>
    ...
    <div class="row">
      <Button style="flex: 1;" @click="applyBackgroundAllSlide()"><IconCheck /> {{ $t('toolbar.applyBackgroundAll') }}</Button>
    </div>
    ...
    <div class="row">
      <Select 
        ...
        :options="[
          { label: $t('toolbar.widescreen') + ' 16 : 9', value: 0.5625 },
          { label: $t('toolbar.widescreen') + ' 16 : 10', value: 0.625 },
          { label: $t('toolbar.standard') + ' 4 : 3', value: 0.75 },
          { label: $t('toolbar.paper') + ' A3 / A4', value: 0.70710678 },
          { label: $t('toolbar.portrait') + ' A3 / A4', value: 1.41421356 },
        ]"
      />
    </div>

    <div class="row">
      <div class="canvas-size">{{ $t('toolbar.canvasSize', { size: `${viewportSize} × ${toFixed(viewportSize * viewportRatio)}` }) }}</div>
    </div>

    <Divider />

    <div class="title">
      <span>{{ $t('toolbar.globalTheme') }}</span>
    ...
    <div class="row">
      <div style="width: 40%;">{{ $t('toolbar.font') }}：</div>
      <Select
        style="width: 60%;"
    ...
    <div class="row">
      <div style="width: 40%;">{{ $t('toolbar.fontColor') }}：</div>
      <Popover trigger="click" style="width: 60%;">
        <template #content>
          <ColorPicker
    ...
    <div class="row">
      <div style="width: 40%;">{{ $t('toolbar.backgroundColor') }}：</div>
      <Popover trigger="click" style="width: 60%;">
        <template #content>
          <ColorPicker
    ...
    <template v-if="moreThemeConfigsVisible">
      ...
      <div class="row" style="height: 30px;">
        <div style="width: 40%;">{{ $t('toolbar.blur') }}：</div>
        <Slider
          style="width: 60%;"
          :min="1"
          :max="20"
    ...
    <div class="row">
      <Button style="flex: 1;" @click="applyThemeToAllSlides(moreThemeConfigsVisible)"><IconCheck /> {{ $t('toolbar.applyThemeAll') }}</Button>
    </div>

    <div class="row">
      <Button style="flex: 1;" @click="applyFontToAllSlides(theme.fontName)"><IconCheck /> {{ $t('toolbar.applyFontAll') }}</Button>
    </div>

    <div class="row">
      <Button style="flex: 1;" @click="themeStylesExtractVisible = true"><IconPlatte /> {{ $t('toolbar.extractTheme') }}</Button>
    </div>
