<template>
  <div class="theme-colors-setting">
    <div class="title">{{ $t('themeColorsSetting.title') }}</div>
    ...
    >
      <template #item="{ element, index }">
        <div class="row">
          <div class="label" style="width: 40%;">{{ $t('themeColorsSetting.label', { index: index + 1 }) }}</div>
    ...
    <Button class="btn" type="primary" @click="setThemeColors()">{{ $t('themeColorsSetting.confirm') }}</Button>
  </div>
</template>
