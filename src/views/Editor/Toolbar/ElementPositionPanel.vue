<template>
  <div class="element-positopn-panel">
    <div class="title">{{ $t('toolbar.order') }}：</div>
    <ButtonGroup class="row">
      <Button style="flex: 1;" @click="orderElement(handleElement!, ElementOrderCommands.TOP)"><IconSendToBack /> {{ $t('toolbar.topOrder') }}</Button>
      <Button style="flex: 1;" @click="orderElement(handleElement!, ElementOrderCommands.BOTTOM)"><IconBringToFrontOne /> {{ $t('toolbar.bottomOrder') }}</Button>
    </ButtonGroup>
    <ButtonGroup class="row">
      <Button style="flex: 1;" @click="orderElement(handleElement!, ElementOrderCommands.UP)"><IconBringToFront /> {{ $t('toolbar.upOrder') }}</Button>
      <Button style="flex: 1;" @click="orderElement(handleElement!, ElementOrderCommands.DOWN)"><IconSentToBack /> {{ $t('toolbar.downOrder') }}</Button>
    </ButtonGroup>

    <Divider />
    
    <div class="title">{{ $t('toolbar.align') }}：</div>
    <ButtonGroup class="row">
      <Button style="flex: 1;" v-tooltip="$t('toolbar.alignLeft')" @click="alignElementToCanvas(ElementAlignCommands.LEFT)"><IconAlignLeft /></Button>
      <Button style="flex: 1;" v-tooltip="$t('toolbar.alignH')" @click="alignElementToCanvas(ElementAlignCommands.HORIZONTAL)"><IconAlignVertically /></Button>
      <Button style="flex: 1;" v-tooltip="$t('toolbar.alignRight')" @click="alignElementToCanvas(ElementAlignCommands.RIGHT)"><IconAlignRight /></Button>
    </ButtonGroup>
    <ButtonGroup class="row">
      <Button style="flex: 1;" v-tooltip="$t('toolbar.alignTop')" @click="alignElementToCanvas(ElementAlignCommands.TOP)"><IconAlignTop /></Button>
      <Button style="flex: 1;" v-tooltip="$t('toolbar.alignV')" @click="alignElementToCanvas(ElementAlignCommands.VERTICAL)"><IconAlignHorizontally /></Button>
      <Button style="flex: 1;" v-tooltip="$t('toolbar.alignBottom')" @click="alignElementToCanvas(ElementAlignCommands.BOTTOM)"><IconAlignBottom /></Button>
    </ButtonGroup>
    ...
    <template v-if="handleElement!.type !== 'line'">
      <div class="row">
        ...
        <template v-if="['image', 'shape', 'audio'].includes(handleElement!.type)">
          <IconLock style="width: 10%;" class="icon-btn active" v-tooltip="$t('toolbar.unlockRatio')" @click="updateFixedRatio(false)" v-if="fixedRatio" />
          <IconUnlock style="width: 10%;" class="icon-btn" v-tooltip="$t('toolbar.lockRatio')" @click="updateFixedRatio(true)" v-else />
        </template>
